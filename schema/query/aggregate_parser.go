...
// MustParseAggregate parses a predicate expression and panics in case of error.
func MustParseAggregate(query string) Aggregate {
	q, err := ParseAggregate(query)
	if err != nil {
		panic(fmt.Sprintf("query: ParseAggregate(%q): %v", query, err))
	}
	return q
}

// ParseAggregate parses a predicate.
func ParseAggregate(aggregate string) (Aggregate, error) {
	if aggregate == "" {
		return Aggregate{}, nil
	}
	p := &aggregateParser{query: aggregate}
	return p.parse()
}

func (p *aggregateParser) parse() (Aggregate, error) {
	p.eatWhitespaces()
...
//   {$or: [{foo: "bar"}, {foo: "baz"}]}
//   {foo: {$exists: true}}
func (p *aggregateParser) parseExpressions() ([]Expression, error) {
	exps := []Expression{}
	if !p.expect('{') {
...
//   $or: [{foo: "bar"}, {foo: "baz"}]
//   foo: {$exists: true}
func (p *aggregateParser) parseExpression() (Expression, error) {
	oldPos := p.pos
...
func (p *aggregateParser) parseSubExpressions() ([]Expression, error) {
	if !p.expect('[') {
		return nil, fmt.Errorf("expected '[' got %q", p.peek())
	}
	subExps := []Expression{}
...
//   {$ne: "foo"}
//   {$in: ["foo", "bar"]}
func (p *aggregateParser) parseCommand(field string) (Expression, error) {
	oldPos := p.pos
	if p.expect('{') {
		p.eatWhitespaces()
		if p.expect('}') {
...
// after the ":".
func (p *aggregateParser) parseLabel() (label string, err error) {
	if p.peek() == '"' {
...
func (p *aggregateParser) parseValue() (Value, error) {
	c := p.peek()
	switch c {
	case '"':
...
func (p *aggregateParser) parseValues() ([]Value, error) {
	if !p.expect('[') {
		return nil, fmt.Errorf("expected '[' got %q", p.peek())
	}
	values := []Value{}
...
func (p *aggregateParser) parseDict() (map[string]Value, error) {
	if !p.expect('{') {
		return nil, fmt.Errorf("expected '{' got %q", p.peek())
	}
...
func (p *aggregateParser) parseBool() (bool, error) {
	switch p.peek() {
	case 't':
...
func (p *aggregateParser) parseNumber() (float64, error) {
	end := p.pos
	if p.peek() == '-' {
		end++
	}
...
func (p *aggregateParser) parseString() (string, error) {
	if p.peek() != '"' {
...
func (p *aggregateParser) peek() byte {
	if p.more() {
...
func (p *aggregateParser) peekAt(pos int) byte {
	if pos < len(p.query) {
		return p.query[pos]
	}
	return 0
...
func (p *aggregateParser) eatWhitespaces() {
	for p.more() {
